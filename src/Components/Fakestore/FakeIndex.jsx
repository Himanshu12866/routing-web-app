...
export default function IndexFake(){
    ...
    return (
        ...
        <nav className="navbar navbar-expand-sm p-3">
            <a className="navbar-brand fs-4" href="products">Shopper.</a>
            <button className="navbar-toggler" data-bs-toggle="collapse" data-bs-target="#items">
            ...
            <ol className="navbar-nav">
                {
                    list.map(item => <li className="nav-item" key={item}>
                        <Link className="nav-link" to= {`products/${item}`}>{item.toUpperCase()}</Link>
                    </li>)
                }
            </ol>
